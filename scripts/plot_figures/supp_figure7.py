...
def supp_figure7(df_orig):
    ...
    supp_figure7b(df_norm_adj)
    supp_figure7c(df_per_adj)
    supp_figure7d(df_per_adj)  # original channel choice
    supp_figure7f(df_norm_orig)
    supp_figure7g(df_per_orig)
    supp_figure7h(df_per_orig)  # original channel choice


def supp_figure7b(df_norm_adj, prefix='B__'):
    """Reproduce Shreve et al."""
    normalized_bands_by_hemisphere(df_norm_adj, fig_dir='Figure_S7',
                                   prefix=prefix)


def supp_figure7c(df_per_adj, prefix='C__'):
    """S7e for periodic power."""
    periodic_bands_by_hemisphere(df_per_adj, fig_dir='Figure_S7',
                                 prefix=prefix)


def supp_figure7d(df_per):
    """Periodic gamma does correlate within subjects for adjacent LFP
    channels."""
    exemplary_subs = cfg.EXEMPLARY_SUBS_GAMMA
    periodic_gamma_within(df_per, fig_dir='Figure_S7', prefix='D1__',
                          exemplary_subs=exemplary_subs, figsize=(1.55, 1.34),
                          bbox_to_anchor=(-.3, 1))
    gamma_peaks_by_hemisphere(df_per, fig_dir='Figure_S7', prefix='D2__')


def supp_figure7f(df_norm_dist, prefix='F__'):
    """Reproduce Shreve et al."""
    normalized_bands_by_hemisphere(df_norm_dist, fig_dir='Figure_S7',
                                   prefix=prefix)


def supp_figure7g(df_per_dist, prefix='G__'):
    """S7e for periodic power."""
    periodic_bands_by_hemisphere(df_per_dist, fig_dir='Figure_S7',
                                 prefix=prefix)


def supp_figure7h(df_per):
    """Periodic gamma does not correlate within subjects for distant LFP
    channels."""
    exemplary_subs = cfg.EXEMPLARY_SUBS_GAMMA
    periodic_gamma_within(df_per, fig_dir='Figure_S7', prefix='H1__',
                          exemplary_subs=exemplary_subs, figsize=(1.55, 1.34),
                          bbox_to_anchor=(-.3, 1))
    gamma_peaks_by_hemisphere(df_per, fig_dir='Figure_S7', prefix='H2__')

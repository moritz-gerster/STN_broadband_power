...
from scripts.plot_figures.settings import LINEWIDTH_AXES, N_PERM_CORR
...
def forest_plot_correlation(df, X, y, n_perm=N_PERM_CORR, r_lim=(-1, 1),
                            ...
                            xlabel=True, output_file=None, title_y=.9,
                            markerscale=0.05, y_fontsize=5.5,
                            figsize=(3, 1.2), fig_dir='Figure2'):
    ...
    for j, x in enumerate(X):
        ...
        for i, project in enumerate(project_order):
            ...
            markersize = sample_size * markerscale
            ...
            ax.errorbar(x_plot, y_plot, **error_kwargs, color=ci_color,
        ...
        if ci_all[0] < 0 < ci_all[1]:
            # remove required n if not significant
        ...
        if xlabel:
            ...
            xlabel = None
        if output_file:
            print(stat_string, file=output_file)
        ax.axvline(0, color="k", linestyle="-", lw=LINEWIDTH_AXES, zorder=0)
        if j > 0:
            ax.yaxis.set_tick_params(width=0, length=0)
        ax.spines['left'].set_visible(False)
        if dataset_labels:
            ticks = positions
            ax.set_yticks(ticks=ticks, labels=labels, fontsize=y_fontsize)
        ...
        title = band_title
        if title_y:
            ax.set_title(title, linespacing=1.3, y=title_y)
        ax.set_xlim(r_lim)
